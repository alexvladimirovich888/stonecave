...
marketplace.addEventListener("click", (e) => {
  if (e.target.classList.contains("card-btn")) {
    ...
    modalPrice.textContent = `Price: $NE ${selectedItem.price}`;
...
function openModal(citizen) {
  ...
  document.getElementById("modal-price").textContent = `Price: $NE ${citizen.price || 0} | Rank: ${rank}`;
...
document.getElementById("sell-btn").addEventListener("click", () => {
  ...
  if (citizenIndex !== -1) {
    ...
    // Показываем сообщение об успехе
    showSuccessMessage(`You sold ${citizen.name} for $NE ${citizen.price || 0}`);    
    document.getElementById("modal").style.display = "none";
  }
});
...
document.getElementById("connect-button").addEventListener("click", () => {
  ...
  // Показываем сообщение об успехе
  showSuccessMessage("Homo revived successfully! You received 1000 $NE");
});
