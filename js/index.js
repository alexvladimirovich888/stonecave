document.getElementById("tg").addEventListener("click", function () {
  const url = "https://x.com"; // Обновленная ссылка на Twitter
...
marketplace.addEventListener("click", (e) => {
  if (e.target.classList.contains("card-btn")) {
    ...
    modalPrice.textContent = `Price: $MEZ ${selectedItem.price}`;
...
function openModal(citizen) {
  ...
  document.getElementById("modal-price").textContent = `Price: $MEZ ${citizen.price || 0} | Rank: ${rank}`;
...
document.getElementById("sell-btn").addEventListener("click", () => {
  ...
  if (citizenIndex !== -1) {
    ...
    // Показываем сообщение об успехе
    showSuccessMessage(`You sold ${citizen.name} for $MEZ ${citizen.price || 0}`);    
    document.getElementById("modal").style.display = "none";
  }
});
...
document.getElementById("connect-button").addEventListener("click", () => {
  ...
  // Показываем сообщение об успехе
  showSuccessMessage("Homo revived successfully! You received 1000 $MEZ");
});
